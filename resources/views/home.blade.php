                ...
                document.addEventListener("DOMContentLoaded", function() {
                    ...
                    $("#btnCalculaRNP").on("click", function() {
                        ...
                        let datosFiltrados = liquidacionesData.filter(item => {
                            // Normalizo campos a string en mayúsculas, evitando null/undefined
                            const transporte = (item.Transporte ?? "").toString().toUpperCase();
                            const variedad = (item.variedad ?? "").toString().toUpperCase();
                            const cliente = (item.cliente ?? "").toString().toUpperCase();
                            const especie = (item.especie ?? "").toString().toUpperCase();
                            const pais = (item.Pais ?? "").toString().toUpperCase();
                            const color = (item.color ?? "").toString().toUpperCase();

                            return (
                                (!transporteSel?.length || transporteSel.includes(transporte)) &&
                                (!variedadesSel?.length || variedadesSel.includes(variedad)) &&
                                // ( !etiquetasSel?.length || etiquetasSel.includes(etiqueta) ) &&
                                (!clientesSel?.length || clientesSel.includes(cliente)) &&
                                (!especiesSel?.length || especiesSel.includes(especie)) &&
                                (!PaisSel || PaisSel.toUpperCase() === pais) &&
                                (!ColorSel || ColorSel.toUpperCase() === color)
                            );
                        });
