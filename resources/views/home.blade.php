...
function transformarDatosParaTreeGrid(data, clientePrincipal, clienteComparado) {
    ...
    data.forEach(row => {
        ...
        if (!grouped[key]) {
            grouped[key] = {
                values: niveles.reduce((acc, n) => ({ ...acc, [n]: row[n] || 'Sin ' + n }), {}),
                clientes: {},
                FOB_TO_USD: 0,
                Kilos_Total: 0
            };
        ...
        if (!grouped[key].clientes[row.cliente]) {
            grouped[key].clientes[row.cliente] = { FOB_TO_USD: 0, Kilos_Total: 0 };
        }
        ...
        niveles.forEach((nivel, i) => {
            ...
            if (!nodeMap.has(id)) {
                ...
                if (i === niveles.length - 1) {
                    ...
                    node.FOB_KG = group.Kilos_Total > 0 ? group.FOB_TO_USD / group.Kilos_Total : 0;
                    node.FOB_Principal = group.clientes[clientePrincipal]?.FOB_TO_USD || 0;
                    node.Kilos_Principal = kilosPrincipal;
                    node.FOB_KG_Principal = node.Kilos_Principal > 0 ? node.FOB_Principal / node.Kilos_Principal : 0;
    ...
    for (let level = niveles.length - 2; level >= 0; level--) {
        treeData.filter(n => n.level === level).forEach(node => {
            ...
            node.Kilos_Principal = children.reduce((sum, c) => sum + (c.Kilos_Principal || 0), 0);
            node.FOB_KG_Principal = node.Kilos_Principal > 0 ? node.FOB_Principal / node.Kilos_Principal : 0;
    ...
    console.log('Primeros nodos:', treeData.filter(n => n.level === 0).slice(0, 2), 'Hijos muestra:', treeData.filter(n => n.parent === treeData[0]?.id).slice(0, 2));
    return treeData;
}
...
function actualizarComparativaFOBCliente(liquidacionesData) {
    ...
    const semanas = [...new Set(liquidacionesData.filter(item => item.ETA).map(item => getISOWeek(item.ETA)).filter(w => w))].sort();
    ...
    ['#clientePrincipalComparativo', '#clienteComparado'].forEach(selector => {
        ...
        console.log(`${selector} opciones agregadas:`, $select.find('option').map((i, opt) => opt.value).get());

        try {
            $select.select2({ width: '100%' });
            if (selector === '#clientePrincipalComparativo' && !currentVal && clientes.includes('Chen Sun')) {
    ...
    if (!$semanaComparativa.length) {
        ...
        const currentVal = Array.isArray($semanaComparativa.val()) ? $semanaComparativa.val() : ($semanaComparativa.val() ? [$semanaComparativa.val()] : []);
        ...
        console.log('#semanaComparativa opciones agregadas:', $semanaComparativa.find('option').map((i, opt) => opt.value).get());

        try {
            $semanaComparativa.select2({ width: '100%' });
    ...
    const treeData = transformarDatosParaTreeGrid(dataFiltrada, filtros.clientePrincipal, filtros.clienteComparado);
    ...
    $("#treegrid-clientes").jqGrid({
        ...
        data: treeData,
        colModel: [
            { name: "name", label: "Nivel", width: 250 },
            { name: "FOB_Principal", label: `${filtros.clientePrincipal} FOB USD`, width: 150, align: "right", formatter: 'currency', formatoptions: { prefix: '$', decimalPlaces: 2 } },
            { name: "Kilos_Principal", label: `${filtros.clientePrincipal} Kilos`, width: 150, align: "right", formatter: 'number', formatoptions: { decimalPlaces: 0 } },
            { name: "FOB_KG_Principal", label: `${filtros.clientePrincipal} FOB/KG`, width: 150, align: "right", formatter: 'currency', formatoptions: { prefix: '$', decimalPlaces: 2 } },
            { name: "FOB_Resto", label: `${filtros.clienteComparado || 'Resto'} FOB USD`, width: 150, align: "right", formatter: 'currency', formatoptions: { prefix: '$', decimalPlaces: 2 } },
            { name: "Kilos_Resto", label: `${filtros.clienteComparado || 'Resto'} Kilos`, width: 150, align: "right", formatter: 'number', formatoptions: { decimalPlaces: 0 } },
            { name: "FOB_KG_Resto", label: "Resto FOB/KG USD", width: 150, align: "right", formatter: 'currency', formatoptions: { prefix: '$', decimalPlaces: 2 } },
            { name: "Diferencia", label: "Diferencia", width: 150, align: "right", formatter: 'currency', formatoptions: { prefix: '$', decimalPlaces: 2 } },
            { name: "TotalDiferencia", label: "Total Diferencia", width: 150, align: "right", formatter: 'currency', formatoptions: { prefix: '$', decimalPlaces: 2 } }
        ],
        treeGrid: true,
        ...
        loadComplete: function() {
            console.log('jqGrid cargado, filas:', $(this).jqGrid('getGridParam', 'records'));
