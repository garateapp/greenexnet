...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.pesoEmbalaje.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.peso-embalajes.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('especie') ? 'is-invalid' : '' }}" name="especie_id" id="especie_id" required>
                    @foreach($especies as $id => $entry)
                        <option value="{{ $id }}" {{ old('especie_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('peso_neto') ? 'is-invalid' : '' }}" type="number" name="peso_neto" id="peso_neto" value="{{ old('peso_neto', '') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('peso_bruto') ? 'is-invalid' : '' }}" type="number" name="peso_bruto" id="peso_bruto" value="{{ old('peso_bruto', '') }}" step="0.01" required>
