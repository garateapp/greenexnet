...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.costo.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.costos.update", [$costo->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nombre">{{ trans('cruds.costo.fields.nombre') }}</label>
                <input class="form-control {{ $errors->has('nombre') ? 'is-invalid' : '' }}" type="text" name="nombre" id="nombre" value="{{ old('nombre', $costo->nombre) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('valor_x_defecto') ? 'is-invalid' : '' }}" type="text" name="valor_x_defecto" id="valor_x_defecto" value="{{ old('valor_x_defecto', $costo->valor_x_defecto) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('categoria') ? 'is-invalid' : '' }}" name="categoria" id="categoria">
                    ...
                    @foreach(App\Models\Costo::CATEGORIA_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('categoria', $costo->categoria) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
