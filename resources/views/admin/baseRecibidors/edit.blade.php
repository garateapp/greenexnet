...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.baseRecibidor.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.base-recibidors.update", [$baseRecibidor->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cliente') ? 'is-invalid' : '' }}" name="cliente_id" id="cliente_id" required>
                    @foreach($clientes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('cliente_id') ? old('cliente_id') : $baseRecibidor->cliente->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('codigo') ? 'is-invalid' : '' }}" type="text" name="codigo" id="codigo" value="{{ old('codigo', $baseRecibidor->codigo) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('rut_sistema') ? 'is-invalid' : '' }}" type="text" name="rut_sistema" id="rut_sistema" value="{{ old('rut_sistema', $baseRecibidor->rut_sistema) }}">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\BaseRecibidor::ESTADO_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('estado') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="estado_{{ $key }}" name="estado" value="{{ $key }}" {{ old('estado', $baseRecibidor->estado) === (string) $key ? 'checked' : '' }}>
