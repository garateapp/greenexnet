...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.diccionario.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.diccionarios.update", [$diccionario->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="variable">{{ trans('cruds.diccionario.fields.variable') }}</label>
                <input class="form-control {{ $errors->has('variable') ? 'is-invalid' : '' }}" type="text" name="variable" id="variable" value="{{ old('variable', $diccionario->variable) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('valor') ? 'is-invalid' : '' }}" type="text" name="valor" id="valor" value="{{ old('valor', $diccionario->valor) }}" required>
                @if($errors->has('valor'))
                    <div class="invalid-feedback">
                        {{ $errors->first('valor') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('tipo') ? 'is-invalid' : '' }}" name="tipo" id="tipo" required>
                    ...
                    @foreach(App\Models\Diccionario::TIPO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('tipo', $diccionario->tipo) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('tipo'))
                    <div class="invalid-feedback">
