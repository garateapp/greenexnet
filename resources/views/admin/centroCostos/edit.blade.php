...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.centroCosto.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.centro-costos.update", [$centroCosto->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('entidad_id') ? 'is-invalid' : '' }}" name="entidad_id" id="entidad_id">
                    @foreach($entidads as $id => $entry)
                        <option value="{{ $id }}" {{ (old('entidad_id') ? old('entidad_id') : $centroCosto->entidad_id) == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('id_centrocosto') ? 'is-invalid' : '' }}" type="text" name="id_centrocosto" id="id_centrocosto" value="{{ old('id_centrocosto', $centroCosto->id_centrocosto) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('c_centrocosto') ? 'is-invalid' : '' }}" type="text" name="c_centrocosto" id="c_centrocosto" value="{{ old('c_centrocosto', $centroCosto->c_centrocosto) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('n_centrocosto') ? 'is-invalid' : '' }}" type="text" name="n_centrocosto" id="n_centrocosto" value="{{ old('n_centrocosto', $centroCosto->n_centrocosto) }}" required>
