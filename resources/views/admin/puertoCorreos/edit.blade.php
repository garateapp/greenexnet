...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.puertoCorreo.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.puerto-correos.update", [$puertoCorreo->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('puerto_embarque') ? 'is-invalid' : '' }}" name="puerto_embarque_id" id="puerto_embarque_id" required>
                    @foreach($puerto_embarques as $id => $entry)
                        <option value="{{ $id }}" {{ (old('puerto_embarque_id') ? old('puerto_embarque_id') : $puertoCorreo->puerto_embarque->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('emails') ? 'is-invalid' : '' }}" name="emails" id="emails">{{ old('emails', $puertoCorreo->emails) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pais') ? 'is-invalid' : '' }}" name="pais_id" id="pais_id">
                    @foreach($pais as $id => $entry)
                        <option value="{{ $id }}" {{ (old('pais_id') ? old('pais_id') : $puertoCorreo->pais->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('pais'))
                    <div class="invalid-feedback">
