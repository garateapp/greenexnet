...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.valorFlete.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.valor-fletes.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="condicion">{{ trans('cruds.valorFlete.fields.condicion') }}</label>
                <input class="form-control {{ $errors->has('condicion') ? 'is-invalid' : '' }}" type="number" name="condicion" id="condicion" value="{{ old('condicion', '0') }}" step="0.1" max="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('valor_dolar') ? 'is-invalid' : '' }}" name="valor_dolar_id" id="valor_dolar_id">
                    @foreach($valor_dolars as $id => $entry)
                        <option value="{{ $id }}" {{ old('valor_dolar_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
