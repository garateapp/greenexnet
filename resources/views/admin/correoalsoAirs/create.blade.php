...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.correoalsoAir.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.correoalso-airs.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('puerto_requerido') ? 'is-invalid' : '' }}" name="puerto_requerido" id="puerto_requerido">{{ old('puerto_requerido') }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('correos') ? 'is-invalid' : '' }}" name="correos" id="correos">{{ old('correos') }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('also_notify') ? 'is-invalid' : '' }}" name="also_notify" id="also_notify">{{ old('also_notify') }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('transporte') ? 'is-invalid' : '' }}" name="transporte" id="transporte" required>
                    ...
                    @foreach(App\Models\CorreoalsoAir::TRANSPORTE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('transporte', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
