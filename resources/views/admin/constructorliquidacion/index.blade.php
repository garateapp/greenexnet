...
@section('content')
    ...
    <script>
        ...
        `;
                            ...
                            const ordenCalibres = ['7J', '6J', '5J', '4J', '3J', '2J', 'J',
                                'XL'
                            ];

                            // Objeto para agrupar por variedad y etiqueta
                            ...
                            $.each(response.result, function(index, item) {
                                ...
                                if (item.categoria.toUpperCase() == 'CAT 1' || item
                                    ...
                                    let totalKilos = parseFloat(item.total_kilos
                                        .replace(
                                            ',', '.')) || 0;
                                    let rnpTotal = parseFloat(item.resultado_total
                                        .replace(
                                            ',', '.')) || 0;
                                    let rnpKilo = parseFloat(item.resultado_kilo
                                        .replace(
                                            ',', '.')) || 0;
                            ...
                            $.each(datosAgrupadosNorma, function(variedad, datosVariedad) {
                                ...
                                etiquetasPorVariedad[variedad] = {
                                    numeroEtiquetas: etiquetasUnicas.length,
...
`;
                            ...
                            $.each(variedades, function(index, variedad) {
                                ...
                                $.each(etiquetas, function(i, etiqueta) {
                                    ...
                                    let calibres = Object.keys(datosEtiqueta
                                        .calibres).sort((a, b) =>
                                        ordenCalibres.indexOf(a) -
                                        ordenCalibres.indexOf(b));
                                    let rowspanEtiqueta = calibres.length;

                                    let isFirstEtiquetaRow = true;

                                    // Generar filas para cada calibre
                                    $.each(calibres, function(j, calibre) {
                                        let datosCalibre = datosEtiqueta
                                            .calibres[calibre];
                                        let curvaCalibre = datosEtiqueta
                                        ...
                                        let etiquetaCell =
                                            isFirstEtiquetaRow ?
                                            `<td rowspan="${rowspanEtiqueta}">${etiqueta}</td>` :
                                            '';
                ...
                <tr>
                    ...
                    <td class="number">${datosCalibre.rnp_total.toFixed(2)}</td>
                    <td class="number">${rnpKilo}</td>
                </tr>
            `;
                                        ...
                                        totalVariedad[variedad]
                                            ...
                                            .total_kilos;
                                        totalVariedad[variedad]
                                            .rnp_total += datosCalibre
                                            .rnp_total;
                                        totalVariedad[variedad]
                                            .rnp_kilo_sum +=
                                            datosCalibre.rnp_kilo *
                                    ...
                                    let rnpKiloEtiqueta = datosEtiqueta
                                        .rnp_kilo_kilos ? (datosEtiqueta
                                            .rnp_kilo_sum / datosEtiqueta
            ...
            <tr class="total-row">
                ...
                <td class="number">${datosEtiqueta.cajas_equivalentes}</td>
                <td class="number">${datosEtiqueta.total_kilos.toFixed(2)}</td>
                <td class="number">${datosEtiqueta.rnp_total.toFixed(2)}</td>
                <td class="number">${rnpKiloEtiqueta}</td>
            </tr>
        `;

                                    // Acumular totales generales
                                    totalGeneralNorma.cajas_equivalentes +=
                                        parseFloat(datosEtiqueta
                                            .cajas_equivalentes);
                                    totalGeneralNorma.total_kilos +=
                                        datosEtiqueta
                                        .total_kilos;
                                    totalGeneralNorma.rnp_total += datosEtiqueta
                                        .rnp_total;
                                    totalGeneralNorma.rnp_kilo_sum +=
                                        datosEtiqueta
                                        .rnp_kilo_sum;
                                    totalGeneralNorma.rnp_kilo_kilos +=
                                        datosEtiqueta
                                        .rnp_kilo_kilos;
                                });
                            });
                            ...
                            $.each(variedades, function(index, variedad) {
                                let rnpKiloVariedad = totalVariedad[variedad]
                                    .rnp_kilo_kilos ? (totalVariedad[variedad]
                                        .rnp_kilo_sum / totalVariedad[variedad]
                                        .rnp_kilo_kilos).toFixed(4) : '0.0000';
                                htmlOutputNorma += `
        <tr class="total-row">
            <td colspan="3">Total ${variedad}</td>
            <td></td>
            <td class="number">${totalVariedad[variedad].cajas_equivalentes.toFixed(0)}</td>
            <td class="number">${totalVariedad[variedad].total_kilos.toFixed(2)}</td>
            <td class="number">${totalVariedad[variedad].rnp_total.toFixed(2)}</td>
            <td class="number">${rnpKiloVariedad}</td>
        </tr>
    `;
                            });
                            ...
                            let rnpKiloGeneral = totalGeneralNorma.rnp_kilo_kilos ? (
                                    totalGeneralNorma
                                    .rnp_kilo_sum / totalGeneralNorma.rnp_kilo_kilos).toFixed(
                                    4) :
                                '0.0000';
                            htmlOutputNorma += `
    <tr class="total-row">
        ...
        <td class="number">${totalGeneralNorma.rnp_total.toFixed(2)}</td>
        <td class="number">${rnpKiloGeneral}</td>
    </tr>
`;
