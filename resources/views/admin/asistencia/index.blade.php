...
<div class="card">
    ...
    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-Asistencium">
            <thead>
                <tr>
                    <th width="10">
                ...
                <tr>
                    ...
                    <td>
                        <select class="search">
                            <option value>{{ trans('global.all') }}</option>
                            @foreach($frecuencia_turnos as $key => $item)
                                <option value="{{ $item->nombre }}">{{ $item->nombre }}</option>
                            @endforeach
                        </select>
                    </td>
...
@endcan
  ...
  let dtOverrideGlobals = {
    ...
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('admin.asistencia.index') }}",
    columns: [
      { data: 'placeholder', name: 'placeholder' },
{ data: 'id', name: 'id' },
{ data: 'locacion_nombre', name: 'locacion.nombre' },
{ data: 'turno_nombre', name: 'turno.nombre' },
{ data: 'personal_nombre', name: 'personal.nombre' },
...
{ data: 'actions', name: '{{ trans('global.actions') }}' }
  ...
  let table = $('.datatable-Asistencium').DataTable(dtOverrideGlobals);
