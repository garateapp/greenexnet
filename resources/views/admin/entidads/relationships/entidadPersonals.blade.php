...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-entidadPersonals">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($personals as $key => $personal)
                        <tr data-entry-id="{{ $personal->id }}">
                            ...
                            <td>
                                @can('personal_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.personals.show', $personal->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('personal_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.personals.edit', $personal->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('personal_delete')
                                    <form action="{{ route('admin.personals.destroy', $personal->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-entidadPersonals:not(.ajaxTable)').DataTable({ buttons: dtButtons })
