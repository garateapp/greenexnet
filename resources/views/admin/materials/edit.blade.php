    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.materials.update", [$material->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('codigo') ? 'is-invalid' : '' }}" type="text" name="codigo" id="codigo" value="{{ old('codigo', $material->codigo) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nombre') ? 'is-invalid' : '' }}" type="text" name="nombre" id="nombre" value="{{ old('nombre', $material->nombre) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('unidad') ? 'is-invalid' : '' }}" name="unidad" id="unidad">
                    ...
                    @foreach(App\Models\Material::UNIDAD_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('unidad', $material->unidad) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('costo_ult_oc') ? 'is-invalid' : '' }}" type="number" name="costo_ult_oc" id="costo_ult_oc" value="{{ old('costo_ult_oc', $material->costo_ult_oc) }}" step="0.01">
