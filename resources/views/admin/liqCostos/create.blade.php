...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.liqCosto.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.liq-costos.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('liq_cabecera') ? 'is-invalid' : '' }}" name="liq_cabecera_id" id="liq_cabecera_id" required>
                    @foreach($liq_cabeceras as $id => $entry)
                        <option value="{{ $id }}" {{ old('liq_cabecera_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nombre_costo') ? 'is-invalid' : '' }}" type="text" name="nombre_costo" id="nombre_costo" value="{{ old('nombre_costo', '') }}" required>
