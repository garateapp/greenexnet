...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.clientesComex.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.clientes-comexes.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="codigo_cliente">{{ trans('cruds.clientesComex.fields.codigo_cliente') }}</label>
                <input class="form-control {{ $errors->has('codigo_cliente') ? 'is-invalid' : '' }}" type="text" name="codigo_cliente" id="codigo_cliente" value="{{ old('codigo_cliente', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nombre_fantasia') ? 'is-invalid' : '' }}" type="text" name="nombre_fantasia" id="nombre_fantasia" value="{{ old('nombre_fantasia', '') }}" required>
