...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.solicitudCompra.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.solicitud-compras.update", [$solicitudCompra->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="titulo">{{ trans('cruds.solicitudCompra.fields.titulo') }}</label>
                <input class="form-control {{ $errors->has('titulo') ? 'is-invalid' : '' }}" type="text" name="titulo" id="titulo" value="{{ old('titulo', $solicitudCompra->titulo) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('descripcion') ? 'is-invalid' : '' }}" name="descripcion" id="descripcion">{{ old('descripcion', $solicitudCompra->descripcion) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_requerida') ? 'is-invalid' : '' }}" type="text" name="fecha_requerida" id="fecha_requerida" value="{{ old('fecha_requerida', $solicitudCompra->fecha_requerida) }}">
            ...
            @if(config('panel.adquisiciones_puede_subir_cotizaciones'))
                <div class="form-group">
                    <div class="form-check {{ $errors->has('cotizaciones_por_adquisiciones') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="checkbox" name="cotizaciones_por_adquisiciones" id="cotizaciones_por_adquisiciones" value="1" {{ old('cotizaciones_por_adquisiciones', $solicitudCompra->cotizaciones_por_adquisiciones) ? 'checked' : '' }}>
