...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.liquidacionesCx.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.liquidaciones-cxes.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="contenedor">{{ trans('cruds.liquidacionesCx.fields.contenedor') }}</label>
                <input class="form-control {{ $errors->has('contenedor') ? 'is-invalid' : '' }}" type="text" name="contenedor" id="contenedor" value="{{ old('contenedor', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('variedad') ? 'is-invalid' : '' }}" name="variedad_id" id="variedad_id" required>
                    @foreach($variedads as $id => $entry)
                        <option value="{{ $id }}" {{ old('variedad_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('pallet') ? 'is-invalid' : '' }}" type="text" name="pallet" id="pallet" value="{{ old('pallet', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('etiqueta') ? 'is-invalid' : '' }}" name="etiqueta_id" id="etiqueta_id">
                    @foreach($etiquetas as $id => $entry)
                        <option value="{{ $id }}" {{ old('etiqueta_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('calibre') ? 'is-invalid' : '' }}" type="text" name="calibre" id="calibre" value="{{ old('calibre', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_venta') ? 'is-invalid' : '' }}" type="text" name="fecha_venta" id="fecha_venta" value="{{ old('fecha_venta') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ventas') ? 'is-invalid' : '' }}" type="number" name="ventas" id="ventas" value="{{ old('ventas', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('precio_unitario') ? 'is-invalid' : '' }}" type="number" name="precio_unitario" id="precio_unitario" value="{{ old('precio_unitario', '') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('monto_rmb') ? 'is-invalid' : '' }}" type="number" name="monto_rmb" id="monto_rmb" value="{{ old('monto_rmb', '') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('liqcabecera') ? 'is-invalid' : '' }}" name="liqcabecera_id" id="liqcabecera_id" required>
                    @foreach($liqcabeceras as $id => $entry)
                        <option value="{{ $id }}" {{ old('liqcabecera_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
