...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.hand-packs.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('embalaje') ? 'is-invalid' : '' }}" name="embalaje" id="embalaje" required>
                    ...
                    @foreach(App\Models\HandPack::EMBALAJE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('embalaje', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('guuid') ? 'is-invalid' : '' }}" type="text" name="guuid" id="guuid" value="{{ old('guuid', '') }}" required>
                @if($errors->has('guuid'))
                    <div class="invalid-feedback">
                        {{ $errors->first('guuid') }}
                    </div>
