...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.capturadorEstructura.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.capturador-estructuras.update", [$capturadorEstructura->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('capturador') ? 'is-invalid' : '' }}" name="capturador_id" id="capturador_id" required>
                    @foreach($capturadors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('capturador_id') ? old('capturador_id') : $capturadorEstructura->capturador->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tipos_seccion_conversors_id') ? 'is-invalid' : '' }}" name="tipos_seccion_conversors_id" id="tipos_seccion_conversors_id" required>
                    @foreach($tipos_seccion_conversors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('tipos_seccion_conversors_id') ? old('tipos_seccion_conversors_id') : $capturadorEstructura->tipos_seccion_conversors_id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('capturador'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('propiedad') ? 'is-invalid' : '' }}" type="text" name="propiedad" id="propiedad" value="{{ old('propiedad', $capturadorEstructura->propiedad) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('coordenada') ? 'is-invalid' : '' }}" type="text" name="coordenada" id="coordenada" value="{{ old('coordenada', $capturadorEstructura->coordenada) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('orden') ? 'is-invalid' : '' }}" type="number" name="orden" id="orden" value="{{ old('orden', $capturadorEstructura->orden) }}" step="1">
                @if($errors->has('orden'))
                    <div class="invalid-feedback">
                        {{ $errors->first('orden') }}
                    </div>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('visible') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="visible" id="visible" value="1" {{ $capturadorEstructura->visible || old('visible', 0) === 1 ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('formula') ? 'is-invalid' : '' }}" type="text" name="formula" id="formula" value="{{ old('formula', $capturadorEstructura->formula) }}">
