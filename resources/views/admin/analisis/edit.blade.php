...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.analisi.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.analisis.update", [$analisi->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('productor') ? 'is-invalid' : '' }}" name="productor_id" id="productor_id">
                    @foreach($productors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('productor_id') ? old('productor_id') : $analisi->productor->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('temporada') ? 'is-invalid' : '' }}" type="text" name="temporada" id="temporada" value="{{ old('temporada', $analisi->temporada) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('especie') ? 'is-invalid' : '' }}" type="text" name="especie" id="especie" value="{{ old('especie', $analisi->especie) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('csg') ? 'is-invalid' : '' }}" type="text" name="csg" id="csg" value="{{ old('csg', $analisi->csg) }}" required>
                @if($errors->has('csg'))
                    <div class="invalid-feedback">
                        {{ $errors->first('csg') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('valor') ? 'is-invalid' : '' }}" type="text" name="valor" id="valor" value="{{ old('valor', $analisi->valor) }}" required>
                @if($errors->has('valor'))
                    <div class="invalid-feedback">
                        {{ $errors->first('valor') }}
                    </div>
