...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.locacions.update", [$locacion->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="nombre">{{ trans('cruds.locacion.fields.nombre') }}</label>
                <input class="form-control {{ $errors->has('nombre') ? 'is-invalid' : '' }}" type="text" name="nombre" id="nombre" value="{{ old('nombre', $locacion->nombre) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cantidad_personal') ? 'is-invalid' : '' }}" type="number" name="cantidad_personal" id="cantidad_personal" value="{{ old('cantidad_personal', $locacion->cantidad_personal) }}" step="1" required>
