...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.politicaCotizacion.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.politica-cotizaciones.store") }}">
            @csrf
            <div class="form-group">
                <label class="required" for="monto_min">{{ trans('cruds.politicaCotizacion.fields.monto_min') }}</label>
                <input class="form-control {{ $errors->has('monto_min') ? 'is-invalid' : '' }}" type="number" name="monto_min" id="monto_min" value="{{ old('monto_min', '') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('monto_max') ? 'is-invalid' : '' }}" type="number" name="monto_max" id="monto_max" value="{{ old('monto_max', '') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cotizaciones_requeridas') ? 'is-invalid' : '' }}" type="number" name="cotizaciones_requeridas" id="cotizaciones_requeridas" value="{{ old('cotizaciones_requeridas', 1) }}" min="1" max="3" required>
