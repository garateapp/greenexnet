...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.metasClienteComex.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.metas-cliente-comexes.update", [$metasClienteComex->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('clientecomex') ? 'is-invalid' : '' }}" name="clientecomex_id" id="clientecomex_id" required>
                    @foreach($clientecomexes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('clientecomex_id') ? old('clientecomex_id') : $metasClienteComex->clientecomex->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cantidad') ? 'is-invalid' : '' }}" type="number" name="cantidad" id="cantidad" value="{{ old('cantidad', $metasClienteComex->cantidad) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('observaciones') ? 'is-invalid' : '' }}" name="observaciones" id="observaciones">{{ old('observaciones', $metasClienteComex->observaciones) }}</textarea>
