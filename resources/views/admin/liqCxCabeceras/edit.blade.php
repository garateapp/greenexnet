...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.liq-cx-cabeceras.update", [$liqCxCabecera->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="instructivo">{{ trans('cruds.liqCxCabecera.fields.instructivo') }}</label>
                <input class="form-control {{ $errors->has('instructivo') ? 'is-invalid' : '' }}" type="text" name="instructivo" id="instructivo" value="{{ old('instructivo', $liqCxCabecera->instructivo) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cliente') ? 'is-invalid' : '' }}" name="cliente_id" id="cliente_id" required>
                    @foreach($clientes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('cliente_id') ? old('cliente_id') : $liqCxCabecera->cliente->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nave') ? 'is-invalid' : '' }}" name="nave_id" id="nave_id" required>
                    @foreach($naves as $id => $entry)
                        <option value="{{ $id }}" {{ (old('nave_id') ? old('nave_id') : $liqCxCabecera->nave->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('nave'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('eta') ? 'is-invalid' : '' }}" type="text" name="eta" id="eta" value="{{ old('eta', $liqCxCabecera->eta) }}">
                @if($errors->has('eta'))
                    <div class="invalid-feedback">
                        {{ $errors->first('eta') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tasa_intercambio') ? 'is-invalid' : '' }}" type="number" name="tasa_intercambio" id="tasa_intercambio" value="{{ old('tasa_intercambio', $liqCxCabecera->tasa_intercambio) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_costo') ? 'is-invalid' : '' }}" type="number" name="total_costo" id="total_costo" value="{{ old('total_costo', $liqCxCabecera->total_costo) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_bruto') ? 'is-invalid' : '' }}" type="number" name="total_bruto" id="total_bruto" value="{{ old('total_bruto', $liqCxCabecera->total_bruto) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_neto') ? 'is-invalid' : '' }}" type="number" name="total_neto" id="total_neto" value="{{ old('total_neto', $liqCxCabecera->total_neto) }}" step="0.01" required>
