    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
             ...
             <div class="form-group">
                <label class="required" for="email">Cargo</label>
            ...
            <div class="form-group">
                <label for="estado_id">{{ trans('cruds.user.fields.estado') }}</label>
                <select class="form-control select2 {{ $errors->has('estado') ? 'is-invalid' : '' }}" name="estado_id" id="estado_id">
                    @foreach($estados as $id => $entry)
                        <option value="{{ $id }}" {{ (old('estado_id') ? old('estado_id') : $user->estado->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
