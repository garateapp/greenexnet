...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.frecuencia-turnos.update', [$frecuenciaTurno->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('dia') ? 'is-invalid' : '' }}" name="dia" id="dia"
                        ...
                        @foreach (App\Models\FrecuenciaTurno::DIA_SELECT as $key => $label)
                            <option value="{{ $key }}"
                                {{ old('dia', $frecuenciaTurno->dia) === (string) $key ? 'selected' : '' }}>
                                {{ $label }}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('dia'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('turno') ? 'is-invalid' : '' }}" name="turno_id"
                        ...
                        @foreach ($turnos as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('turno_id') ? old('turno_id') : $frecuenciaTurno->turno->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('turno'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('nombre') ? 'is-invalid' : '' }}" type="text"
                        name="nombre" id="nombre" value="{{ old('nombre', $frecuenciaTurno->nombre) }}">
