...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.recepcion.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.recepcions.update", [$recepcion->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('productor') ? 'is-invalid' : '' }}" name="productor_id" id="productor_id" required>
                    @foreach($productors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('productor_id') ? old('productor_id') : $recepcion->productor->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('variedad') ? 'is-invalid' : '' }}" type="text" name="variedad" id="variedad" value="{{ old('variedad', $recepcion->variedad) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_kilos') ? 'is-invalid' : '' }}" type="number" name="total_kilos" id="total_kilos" value="{{ old('total_kilos', $recepcion->total_kilos) }}" step="0.01" required>
