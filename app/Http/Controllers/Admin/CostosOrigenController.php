...
class CostosOrigenController extends Controller{
    public function costosorigen(Request $request)
    {
        ...
        DB::raw('SUM(agenciamiento) as agenciamiento'),
            ...
            DB::raw('SUM(otros_documental) as otros_documental'))->get();

        $costos_de_carga=collect();
        $costos_de_embarque_en_origen=collect();
        $costos_de_BL=collect();
        $costos_de_remision_documental=collect();
        ...
        return view('admin.comex.costosorigen', compact('costos'));
    }
